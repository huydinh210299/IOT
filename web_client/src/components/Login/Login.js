...
const LoginRegister = (props) => {
  ...
  const listContentTabs = {
    Login: (<Form
      ...
      <Form.Item
        ...
        rules={[{
          type: 'email',
          required: true, message: 'Your email is invalid!'
        }]}>
        <Input />
      </Form.Item>
      ...
      <Form.Item
        ...
        rules={[{
          type: 'string',
          min: 8,
          required: true, message: 'Must be 8 characters!'
        }]}>
        <Input.Password />
      </Form.Item>
      <Form.Item>
  ...
  if (isAuthenticated === true) {
    ...
    if (history.location.state && history.location.state.from) {
      ...
      return (
        <Redirect to={{ pathname: "/home", state: { from: history.location } }} />
      );
  } else
    return (
      <div style={{ display: "flex", justifyContent: 'center', alignItems: 'center', height: '100%' }}>
        ...
        {isRequesting ? <Spin spinning={isRequesting}>
          <Card
